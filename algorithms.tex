...
When using traditional PSRs in online environments, one typically updates the state vector (weighting on states) when a new observation is added. This eliminates repeatedly transforming the initial state over the observation history when making predictions. Conveniently, the dynamic programming algorithm for minimizing the length of encodings provides a natural way to do state updates for M-PSRs. To do so we cache state vectors from the past along with their encoding length. When new observations are read, we determine the minimal encoding for the extended observation string in the same way as was done in the dynamic programming algorithm in the section above. Because this minimization is over the valid sequences in $\Sigma'$ (valid meaning that the sequence is a suffix of the observation history), one needs to cache at most $\max_{s \in \Sigma'} |s|$ states vectors. 
